...
export default function Onboarding(){
  ...
  const doProfile=async()=>{
    ...
    try{const res=await fetch('http://localhost:8080/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(profile)});const data=res.ok?await res.json():profile;localStorage.setItem('user',JSON.stringify(data));}
    catch{localStorage.setItem('user',JSON.stringify(profile));}
      ...
      <div style={s.progress}>
        {labels.map((label,i)=>(
          <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <span style={{fontSize:12,fontWeight:500,color:step>=i+1?'var(--text-primary)':'var(--text-muted)'}}>{label}</span>
            {i<2&&<div style={{width:36,height:1,background:step>i+1?dotColors[i]:'var(--border)',margin:'0 4px',transition:'background .3s'}}/>}
          </div>
        ))}
      </div>
      ...
      {step===1&&(
        <div style={{display:'flex',gap:8,flexWrap:'wrap',justifyContent:'center',marginTop:20}}>
          {['6 vitals tracked','AI analysis','FDA drug database','Progress trends'].map((f,i)=>(
            <span key={i} style={{padding:'5px 14px',borderRadius:999,fontSize:12,fontWeight:500,background:'var(--surface)',border:'1px solid var(--border)',color:'var(--text-secondary)'}}>{f}</span>
          ))}
        </div>
      )}
    </div>
...
const s={
  page:{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 24px',background:'var(--bg)',fontFamily:"'DM Sans',sans-serif"},
  ...
  card:{width:'100%',maxWidth:520,background:'var(--surface)',borderRadius:24,border:'1px solid var(--border)',padding:'36px 40px',boxShadow:'var(--shadow-md)'},
